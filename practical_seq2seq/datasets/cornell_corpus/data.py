...
'''
def get_id2line():
    lines=open('datasets/cornell_corpus/raw_data/movie_lines.txt', encoding='utf-8', errors='ignore').read().split('\n')
    id2line = {}
    for line in lines:
        _line = line.split(' +++$+++ ')
        if len(_line) == 5:
...
def get_conversations():
    conv_lines = open('datasets/cornell_corpus/raw_data/movie_conversations.txt', encoding='utf-8', errors='ignore').read().split('\n')
    convs = [ ]
...
def process_data():
    ...
    # save them
    np.save('datasets/cornell_corpus/idx_q.npy', idx_q)
    np.save('datasets/cornell_corpus/idx_a.npy', idx_a)
    ...
    with open('datasets/cornell_corpus/metadata.pkl', 'wb') as f:
