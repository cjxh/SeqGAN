...
def main():
    ...
    mnist = input_data.read_data_sets(positive_file) 
    ...
    if not os.path.exists('./save/'+TIME+'-maligan'):
        os.makedirs('./save/'+TIME+'-maligan')
    with open('./save/'+TIME+'-maligan/maligan-config-'+TIME+'.yaml', 'w') as f:
        yaml.dump(GCONFIG, f)
        yaml.dump(DCONFIG, f)
        ...
        for _ in range(5):
            ...
            for batch in dis_batches:
                try:
                    ...
                    # change update grad to take grad. ascend of discriminator loss
                    _, step = sess.run([dis_train_op, dis_global_step], feed)
                except ValueError:
                    pass
